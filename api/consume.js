...
async function tryConsume(uid) {
  // Fetch current state
  const getUrl = `${process.env.SUPABASE_URL}/rest/v1/users?uid=eq.${uid}&select=uid,credits,free_used`;
  ...
  if (rows.length === 0) {
    // Auto-create
    user = { uid, credits: 0, free_used: false };
    await patchUser(uid, { credits: 0, free_used: false }, true);
  } else {
    user = rows[0];
  }
  ...
  if (!user.free_used) {
    // Grant free use
    await patchUser(uid, { free_used: true });
    return { ok: true, creditsLeft: user.credits, usedFree: true };
  }

  if (user.credits > 0) {
    await patchUser(uid, { credits: user.credits - 1 });
...
async function patchUser(uid, patch, isInsert = false) {
  ...
  await fetch(url, {
    ...
    body: JSON.stringify(isInsert ? { uid, ...patch } : patch)
  });
}